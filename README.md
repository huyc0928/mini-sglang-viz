...
**时序回放**：八条流程逐步走，每一步都定位到真实的行号并显示那段源码。流程包括在线请求从 HTTP 到 SSE、离线单进程路径、一个 prefill 步、一个 decode 步（含 overlap 调度的双流与事件同步）、CUDA Graph 的捕获与回放、chunked prefill 跨轮推进、Radix 缓存的匹配与淘汰、张量并行里一次 all_reduce 的两个 rank 时序。
